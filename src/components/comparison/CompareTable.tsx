  ...
  const specs: RowSpec[] = [
    {
      ...
      render: (row) => <span className="font-head font-semibold">{row.provider.name}</span>,
    },
    {
      key: 'price',
    ...
    {
      key: 'data',
      compare: (row) => (row.plan.isUnlimited ? 'unlimited' : String(row.plan.dataAmountMb)),
      render: (row) =>
        row.plan.isUnlimited ? (
          dict.units.unlimited
        ) : (
          <Ltr className="tnum">{formatData(row.plan.dataAmountMb, locale)}</Ltr>
        ),
    },
    {
      key: 'validity',
      ...
      render: (row) => (
        <Ltr className="tnum">
          {interpolate(dict.filters.daysTemplate, { days: row.plan.validityDays })}
        </Ltr>
      ),
    },
    {
      key: 'perUnit',
      compare: (row) => String(row.pricePerGbMinor ?? row.pricePerDayMinor ?? 0),
      render: (row) =>
        row.pricePerGbMinor !== null ? (
          ...
          <Ltr className="tnum">
            {interpolate(dict.plan.perDayTemplate, {
              price: formatPrice(row.pricePerDayMinor, row.price.currency, locale),
            })}
          </Ltr>
        ) : (
          '—'
    ...
    {
      key: 'network',
      compare: (row) => row.plan.networks.map((network) => network.operator).join('+'),
      render: (row) => row.plan.networks.map((network) => network.operator).join(' + ') || '—',
    },
    {
      key: 'technology',
      compare: (row) => String(hasTechnology(row.plan.networks, '5G')),
      render: (row) => (hasTechnology(row.plan.networks, '5G') ? yes : no),
    },
    { key: 'hotspot', compare: (row) => String(row.plan.hotspot), render: (row) => (row.plan.hotspot ? yes : no) },
    { key: 'calls', compare: (row) => String(row.plan.calls), render: (row) => (row.plan.calls ? yes : no) },
    { key: 'sms', compare: (row) => String(row.plan.sms), render: (row) => (row.plan.sms ? yes : no) },
    { key: 'topUp', compare: (row) => String(row.plan.topUp), render: (row) => (row.plan.topUp ? yes : no) },
    {
      key: 'fairUsage',
        ...
        <table className="w-full min-w-[520px] table-fixed border-collapse text-[0.9375rem]">
          ...
          <tbody>
            {specs.map((spec) => {
              ...
              return (
                <tr key={spec.key} className={cn('border-b border-line-soft', !differs && 'text-ink-3')}>
                  <th
                    scope="row"
                  ...
                  {rows.map((row) => (
                    <td
                      key={row.plan.id}
                      className={cn('py-3 pe-3 align-top', differs && 'bg-teal-50/60 font-medium text-ink')}
                    >
                      {spec.render(row)}
                    </td>
                  ))}
