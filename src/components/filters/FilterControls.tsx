...
import type { FeatureKey, FilterOptions, PlanFilters } from '@/lib/comparison/filter';
        ...
        <div className="flex flex-col">
          {options.dataGb.map((gb) => (
            <Checkbox
              ...
              onChange={() => onChange({ ...filters, data: toggle(filters.data, String(gb)) })}
            />
          ))}
          {options.hasUnlimited ? (
            <Checkbox
              ...
              onChange={() => onChange({ ...filters, data: toggle(filters.data, 'unlimited') })}
            />
          ) : null}
        </div>
      </Group>
      ...
      <Group title={dict.filters.validity}>
        <div className="flex flex-col">
          {options.validity.map((days) => (
            <Checkbox
              key={days}
              label={<Ltr className="tnum">{interpolate(dict.filters.daysTemplate, { days })}</Ltr>}
              checked={filters.validity.includes(days)}
              onChange={() => onChange({ ...filters, validity: toggle(filters.validity, days) })}
            />
          ))}
        </div>
      </Group>
      ...
      <Group title={dict.filters.network}>
        <div className="flex flex-col">
          <Checkbox
            ...
            onChange={(event) => onChange({ ...filters, onlyFiveG: event.target.checked })}
          />
          {options.operators.map((operator) => (
            <Checkbox
              ...
              onChange={() => onChange({ ...filters, operators: toggle(filters.operators, operator) })}
            />
          ))}
        </div>
      </Group>
        ...
        <div className="flex flex-col">
          {featureKeys.map((feature: FeatureKey) => (
            <Checkbox
              ...
              onChange={() => onChange({ ...filters, features: toggle(filters.features, feature) })}
            />
          ))}
        </div>
      </Group>
        ...
        <div className="flex flex-col">
          {options.providers.map((provider) => (
            <Checkbox
              ...
              onChange={() => onChange({ ...filters, providers: toggle(filters.providers, provider.id) })}
            />
          ))}
        </div>
      </Group>
      ...
      <Group title={dict.filters.price}>
        ...
        <p className="tnum mt-1 text-[0.8125rem] text-ink-2">
          <Ltr>
            ...
            {formatPrice(filters.maxPriceMinor ?? options.maxPriceMinor, currency, locale)}
          </Ltr>
        </p>
      </Group>
    </div>
